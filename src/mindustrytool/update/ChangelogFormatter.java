...
/**
 * Pure changelog formatting extracted from {@code UpdateService.fetchReleasesAndShowDialog}.
 * No network, no Arc, no bundle dependency — pure Java for unit testing.
 */
public final class ChangelogFormatter {
    ...
    /**
     * Formats raw GitHub releases JSON string (array) into Mindustry markup.
     * Caps at 20 entries.
     */
    ...
    public static String formatReleases(List<ReleaseInfo> releases, ZoneId zoneId, DateTimeFormatter formatter) {
        ...
        for (ReleaseInfo release : releases) {
            ...
            changelog.append("[gold]Download count: ").append(release.downloadCount).append("[white]\n");

            String body = release.body != null && !release.body.isEmpty()
                    ? release.body
                    : "No description provided.";
