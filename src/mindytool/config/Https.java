...
public class Https {
    ...
    /** @return a new HttpsRequest that must be configured & submitted. */
    public static HttpsRequest request(HttpMethod method, String url) {
        if (url == null)
            throw new NullPointerException("url cannot be null.");
        return new HttpsRequest(method).url(url);
    }

    /** @return a new GET HttpsRequest that must be configured & submitted. */
    public static HttpsRequest get(String url) {
        if (url == null)
            throw new NullPointerException("url cannot be null.");
        return new HttpsRequest(HttpMethod.GET).url(url);
    }

    /** Creates and submits a HTTP GET request. */
    public static void get(String url, ConsT<HttpsResponse, Exception> callback) {
        get(url).submit(callback);
    }

    /** Creates and submits a HTTP GET request, with an error handler. */
    public static void get(String url, ConsT<HttpsResponse, Exception> callback, Cons<Throwable> error) {
        get(url).error(error).submit(callback);
    }

    /** @return a new POST HttpsRequest that must be configured & submitted. */
    public static HttpsRequest post(String url) {
        return post(url, (String) null);
    }

    /** Creates and submits a HTTP POST request. */
    public static void post(String url, ConsT<HttpsResponse, Exception> callback) {
        post(url).submit(callback);
    }

    /** @return a new POST HttpsRequest that must be configured & submitted. */
    public static HttpsRequest post(String url, String content) {
        if (url == null)
            throw new NullPointerException("url cannot be null.");
        return new HttpsRequest(HttpMethod.POST).url(url).content(content);
    }
    ...
    public static class HttpsResponse {
        ...
        protected HttpsResponse(HttpURLConnection connection) throws IOException {
            this.connection = connection;
            this.status = HttpStatus.byCode(connection.getResponseCode());
        }

        /**
         * @return the length of received content in bytes as a long. May throw an
         *         exception (?)
         */
         ...
         *         operation was canceled/terminated abnormally. The
         *         timeout is specified when creating the HTTP request, with
         *         {@link HttpsRequest#timeout(int)}
         */
         ...
         * Warning:</b> Do not store a reference to this InputStream. The underlying
         * HTTP connection will be closed after that
         * callback finishes executing. Reading from the InputStream after it's
         * connection has been closed will lead to exception.
         * 
         * @return An {@link InputStream} with the {@link HttpsResponse} data.
         */
        ...
        /**
         * @return the {@link HttpStatus} containing the statusCode of the HTTP
         *         response.
         */
        ...
        /**
         * @return the value of the header with the given name as a {@link String}, or
    ...
    public static class HttpsRequest {
        ...
        /**
         * The time to wait for the HTTP request to be processed, use 0 to block until
         * it is done. The timeout is used for both
         * the timeout when establishing TCP connection, and the timeout until the first
         * byte of data is received.
         */
        public int timeout = 2000;

        /**
         * The content to be used in the HTTP request: A string encoded in the
         * corresponding Content-Encoding set in the headers, with the data to send with
         * the
         * HTTP request. For example, in case of HTTP GET, the content is used as the
         * query string of the GET while on a
         * HTTP POST it is used to send the POST data.
         */
        public String content;

        /**
         * The content as a stream to be used for a POST for example, to transmit custom
         * data.
         */
         ...
         * changed in the web backend because this uses
         * XmlHttpsRequests which always redirect.
         */
        ...
        protected HttpsRequest(HttpMethod method) {
            this.method = method;
        }

        public HttpsRequest error(Cons<Throwable> failed) {
            errorHandler = failed;
            return this;
        }

        public HttpsRequest method(HttpMethod method) {
            this.method = method;
            return this;
        }

        public HttpsRequest url(String url) {
            this.url = url;
            return this;
        }
        ...
        public HttpsRequest redirects(boolean followRedirects) {
            this.followRedirects = followRedirects;
            return this;
        }

        public HttpsRequest credentials(boolean includeCredentials) {
            this.includeCredentials = includeCredentials;
            return this;
        }

        public HttpsRequest header(String name, String value) {
            headers.put(name, value);
            return this;
        }

        public HttpsRequest content(String content) {
            this.content = content;
            return this;
        }

        public HttpsRequest content(InputStream contentStream, long contentLength) {
        ...
        public void submit(ConsT<HttpsResponse, Exception> success) {
            Https.exec.submit(() -> block(success));
        }
    ...
    public static class HttpStatusException extends RuntimeException {
        ...
        public HttpStatusException(String message, HttpStatus status, HttpsResponse response) {
            super(message);
            this.status = status;
    ...
    /** Provides all HTTP methods to use when creating a {@link HttpsRequest}. */
